...
```sql 
SELECT smoking, COUNT(*) FROM healthcare WHERE trim(smoking) is NOT '' GROUP BY smoking;
```
...
```sql 
SELECT is_drinking, COUNT(*) FROM healthcare WHERE trim(is_drinking) is NOT '' GROUP BY is_drinking;
```
...
SELECT is_drinking, COUNT(*) FROM healthcare WHERE trim(blood_pressure) is NOT '' AND blood_pressure >= 200 GROUP BY is_drinking;
```
...
```sql 
SELECT is_drinking, AVG(waist), COUNT(*) FROM healthcare WHERE trim(is_drinking) is NOT '' GROUP BY is_drinking;
``` 
...
```sql
SELECT is_drinking, smoking, AVG(weight / (height * 0.01) /(height * 0.01)) AS '평균 BMI' FROM healthcare WHERE trim(is_drinking) is NOT '' AND trim(smoking) is NOT '' GROUP BY is_drinking, smoking;
```
