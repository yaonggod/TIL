...
```sql
SELECT playlist_track.PlaylistId, Name FROM playlist_track INNER JOIN tracks ON playlist_track.TrackId = tracks.TrackId ORDER BY playlist_track.PlaylistId DESC LIMIT 10;
```  
...
```sql
SELECT playlist_track.PlaylistId, Name FROM playlist_track INNER JOIN tracks ON playlist_track.TrackId = tracks.TrackId WHERE playlist_track.PlaylistId = '10' ORDER BY Name DESC LIMIT 5;
``` 
...
```plain
tracks의 작곡가가 여러명이서 공동 작곡한 경우나 artist에 등록되지 않은 작곡가인 경우가 있어서, 혹은 NULL때문에
```
...
```sql
SELECT CustomerId, COUNT(*) AS '개수' FROM invoice_items JOIN invoices ON invoice_items.InvoiceId = invoices.InvoiceId GROUP BY CustomerId ORDER BY CustomerId LIMIT 5;
```
